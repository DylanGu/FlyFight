...
def get_Class_Path():
    return 'projects/FlyFight/flyfight_branch/frameworks/runtime-src/Classes'
...
def _check_cocos2d_root_env():
    ''' Checking the environment COCOS2D_ROOT_3_1, which will be used for building
    '''

    try:
        COCOS2D_ROOT_3_1 = os.environ['COCOS2D_ROOT_3_1']
    except Exception:
        print "COCOS2D_ROOT_3_1 not defined. Please define COCOS2D_ROOT_3_1 in your environment."
        sys.exit(1)

    return COCOS2D_ROOT_3_1
...
def main():
    ...
    cocos2d_x_root = _check_cocos2d_root_env()
    ...
    #here need change.
    fly_root = os.path.abspath(os.path.join(project_root, get_Class_Path()))
    ...
    config.set('DEFAULT', 'fly_root', fly_root)
    ...
    try:
        ...
        tolua_root = '%s/tools/tolua/fly_fight' % project_root
        output_dir = '%s/game/script_export' % fly_root

        cmd_args = {'fly_fight_logic.ini' : ('fly_fight', 'fly_fight_logic'), \
                    }
        target = 'lua'
